...
int main() {
    ...
    while(1) {
        ...
        if (nave_get_posx(jugador) <= 3 || nave_get_posx(jugador) >= VENTANA_ANCHO) {
    ...
    // BEGIN código del alumno
    polilinea_destruir(polis_nave_con_chorro[0]);
    polilinea_destruir(polis_nave_con_chorro[1]);
    free(jugador);
    // END código del alumno
